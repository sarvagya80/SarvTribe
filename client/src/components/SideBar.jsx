...
function SideBar({ sideBarOpen, setSideBarOpen }) {
  ...
  // If the user object from your backend hasn't loaded yet, show a spinner.
  if (!user) {
    return (
      <div className={`w-60 xl:w-72 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-0 bottom-0 z-20 ${sideBarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
        {/* Use the Loading component for a better visual experience */}
        <Loading height="100%" />
      </div>
    );
  }
    ...
    <div className={`w-60 xl:w-72 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-0 bottom-0 z-20 ${sideBarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      ...
      <div className='w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <div>
            {/* Use full_name for consistency */}
            <h1 className='text-sm font-medium'>{user.full_name}</h1>
            <p className='text-xs text-gray-500'>@{user.username}</p>
          </div>
        </div>
        {/* The separate LogOut icon is removed to simplify the UI, as UserButton handles this. */}
      </div>
    </div>
  );
}
