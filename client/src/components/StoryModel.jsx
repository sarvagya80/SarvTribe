...
import { ArrowLeft, Sparkle, Upload } from 'lucide-react'; // Importing ArrowLeft icon from lucide-react
import { TextIcon } from 'lucide-react'; // Importing TextIcon from lucide-react
import toast from 'react-hot-toast'; // Importing toast for notifications
import { assets } from '../assets/assets';

 // Assuming you have a StoryModal component for creating stories
    ...
    <div className='fixed inset-0 z-110 min-h-screen  bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
      <div className='w-full max-w-md'> 
        ...
        <div className='flex mt-4 gap-2'>
            { bgColors.map((color) => (
                <div key={color} className='w-6 h-6 rounded-full ring cursor-pointer' style={{ backgroundColor: color }} onClick={() => setBackground(color)} />
            ))}
        </div>
        <div className='flex gap-2 mt-4'> 
            ...
            <label  className={`flex-1 flex items-center justify-center gap-2 p-2  cursor-pointer ${mode==='media' ? 'bg-white text-black' : "bg-zinc-800"}`} >
